...
(function(exports) {
  ...
  var 
    ...
    fuzzy = function (obj) {
      ...
      else if (obj instanceof CanvasRenderingContext2D) { // is canvas context
        _initCtx(obj);
      }
      else {
        return null;
  ...
  fuzzy.draw = function (options) {
    ...
    if(options.img && options.img instanceof HTMLImageElement) {
      ...
      if((widthAttr && widthAttr.value != _width )|| (heightAttr && heightAttr.value != _height)) {
        newCanvas = document.createElement('canvas');
        newCanvas.width = widthAttr.value;
        newCanvas.height = heightAttr.value;
        
        newCanvas.getContext("2d").drawImage(_canvas, 0, 0, widthAttr.value, heightAttr.value);
      }
  ...
  function _initCtx(ctx) {
    var tempContext, data;

    _originalCanvas = ctx.canvas;
    _width = _canvas.width = _originalCanvas.width;
    _height = _canvas.height = _originalCanvas.height;
    tempContext = ctx;
    data = tempContext.getImageData(0, 0, _width, _height);
    _context.putImageData(data, 0, 0);
  }
